...
object ReplyNotify {
    ...
    /** 震动节奏（0ms 起振、250ms 振、150ms 停、250ms 振） */
    private val VIBRATION_PATTERN = longArrayOf(0L, 250L, 150L, 250L)

    /**
     * AI 回复完成时调用（[com.pient.app.data.ChatState] 的成功出口：发送一轮之后）。
     *
    ...
    /** 渠道 id = 前缀 + 档位后缀（`_sound_vibration / _sound / _vibration / _silent`） */
            ...
            if (nm.getNotificationChannel(id) == null) {
                nm.createNotificationChannel(
                    NotificationChannel(
                        ...
                        // 顺序要紧：setVibrationPattern 会把渠道的「震动」标志一并置真，
